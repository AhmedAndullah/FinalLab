...
const QuizResultScreen = ({ route, navigation }) => {
  ...
  const handleAnswerPress = (question, correctAnswer, selectedOption) => {
    // Navigate to a detailed explanation screen or handle the press event
    navigation.navigate('AnswerDetail', { question, correctAnswer, selectedOption });
  };
          ...
          {incorrectAnswers.length > 0 && (
            <View style={styles.incorrectAnswersContainer}>
              ...
              {incorrectAnswers.map((item, index) => (
                <TouchableOpacity key={index} style={styles.incorrectAnswer} onPress={() => handleAnswerPress(item.question, item.correctAnswer, item.selectedOption)}>
